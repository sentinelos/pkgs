...
# apk add \
#	abuild apk-tools sentinelos-conf busybox fakeroot syslinux xorriso
...
build_profile() {
	...
	# Defaults
	[ -n "$image_name" ] || image_name="sentinelos-${PROFILE}"
	...
	if [ "$_dirty" = "yes" -o ! -e "$DESTDIR" ]; then
		...
		if [ -z "$_simulate" ]; then
			...
			echo "${image_name}-${RELEASE} ${build_date}" > "$DESTDIR"/.sentinelos-release
		fi
	fi
