...
section_kernels() {
	...
	for _f in $kernel_flavors; do
		...
		local id=$( (echo "$initfs_features::$_hostkeys" ; apk fetch --root "$APKROOT" --simulate sentinelos-base $_pkgs | sort) | checksum)
...
grub_gen_earlyconf() {
	cat <<- EOF
	search --no-floppy --set=root --label "sentinelos-${profile_abbrev:-$PROFILE} $RELEASE $ARCH"
...
create_image_iso() {
	...
	if [ "$ARCH" = ppc64le ]; then
		grub-mkrescue --output ${ISO} ${DESTDIR} -follow-links \
			-sysid LINUX \
			-volid "sentinelos-${profile_abbrev:-$PROFILE} $RELEASE $ARCH"
	else
		if [ "$ARCH" = s390x ]; then
